...
{
    public partial class Raw_Team_Feeder_Clubs : Form
    {
        public Raw_Team_Feeder_Clubs()
        {
            InitializeComponent();
        }
        ...
        private void SaveChangers()
        {
            for (int i = 0; i < Global.team_amount; i++)
            {
                for (int j = 0; j < 7; j++)
                {
                    Global.team[Convert.ToInt32(dataGridView1.Rows[i].Cells[0].Value)].feederClubs[j].isFeederClubs = false;
                }
            }

            for (int i = 0; i < Global.team_amount; i++)
            {
                Global.team[Convert.ToInt32(dataGridView1.Rows[i].Cells[0].Value)].feederClubsAmount = Convert.ToByte(dataGridView1.Rows[i].Cells[4].Value);

                for (int j = 0; j < Global.team[i].feederClubsAmount; j++)
                {
                    if (!(dataGridView1.Rows[i].Cells[j + 5].Value is DBNull))
                    {
                        Global.team[Convert.ToInt32(dataGridView1.Rows[i].Cells[0].Value)].feederClubs[j].isFeederClubs = true;
                        Global.team[Convert.ToInt32(dataGridView1.Rows[i].Cells[0].Value)].feederClubs[j].feederClubsId = Convert.ToInt32(dataGridView1.Rows[i].Cells[j + 5].Value);
                    }
                }
